...
if __name__ == "__main__":
    ...
    for i in range(10):
        ...
        run = wandb.init(config=config, project="VAE-GAN-TEST-LOOP-FREEZE")
        config = wandb.config
        ...
        gan.generador.summary()
        gan.generador.trainable = False
        gan.generador.summary()
        history = gan.fit(Xtrain, batch_size=config.BATCH_SIZE, epochs=5, verbose=0,
                    callbacks=[CallBackHacedor(n=10), WandbCallback()])
        gan.generador.trainable = True
        gan.generador.summary()
